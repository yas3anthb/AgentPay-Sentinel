...
```
  ...
  AI shopping agent                       ▼
        ...
        └───────────▶│    + edge rate limit             scope · revoke   │
                     ...
                     │ 3 Content security analyzer      rules+sim+Groq    │
...
```bash
cp .env.example .env         # add GROQ_API_KEY, or see "Running without a key" below
...
It runs in its own image and venv: CrewAI's tree pins pydantic 2.12 and its
own LiteLLM stack against this gateway's 2.10.4 and the `groq` SDK. See
...
3. **LLM classifier** (`gateway/analyzer/llm.py`, Groq) — generalises furthest.
...
detection layers, not zero.** When the Groq call times out, rate-limits, returns malformed
...
requests where Groq exceeds its timeout, and under an unconditional block every one of
...
- **Circuit breaker** (`gateway/analyzer/llm.py`) — after a few consecutive transport
  failures the Groq call is skipped for a cooldown, so an outage stops costing every
...
The default `.env.example` runs the classifier **live**: add `GROQ_API_KEY` and go. Without
...
- **The web console is unauthenticated**, and its `/api/control/*` route attaches the admin key
  to whatever the browser sends. Locally that is fine — the console is a trusted demo tool. If
  `:3000` is ever exposed, it must sit behind auth, because "can load the page" would otherwise
  mean "can mint delegation tokens and reset state". The gateway's own `/v1/payment-intents`
  still requires a signed delegation token regardless.
- **`POST /v1/approvals/{id}/grant` has no auth** — it stands in for a human tapping *approve*
  in their banking app. What *is* enforced is the binding: the granted token is signed over the
  exact amount, merchant, currency and cart the human saw, and `approvals.rego` re-checks all
  four (`test_approval_is_bound_to_what_the_human_saw`). The demo agent cannot reach this
  endpoint — its only tool is `propose_payment_intent` — and the Telegram flow shows the
  intended shape: the human taps Approve on their phone and the bot, not the agent, calls
  grant. A real deployment binds grant to an authenticated human session.
...
gateway/
  ...
  analyzer/        stage 3: rules.py · similarity.py · llm.py (Groq, circuit breaker) · trust.py
