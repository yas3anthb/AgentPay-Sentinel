"""The Groq call's contract: fixed system prompt, nonce-delimited data block,
...
async def test_call_uses_json_schema_structured_output(groq_stub):
    completions = groq_stub(
        {
...
async def test_untrusted_text_never_enters_the_system_prompt(groq_stub):
    completions = groq_stub(
...
async def test_data_block_delimiter_is_unguessable_and_per_request(groq_stub):
    completions = groq_stub(
        {"injection_detected": False, "confidence": 0.0, "signals": [], "recommended_action": "ALLOW"}
    )
...
async def test_out_of_range_confidence_is_clamped(groq_stub):
    groq_stub(
...
async def test_unknown_recommended_action_defaults_to_block(groq_stub):
    groq_stub(
        {
...
async def test_truncated_response_is_degraded_not_clean(groq_stub):
    groq_stub(
        {"injection_detected": False, "confidence": 0.0, "signals": [], "recommended_action": "ALLOW"},
...
async def test_circuit_opens_after_consecutive_transport_failures(failing_groq):
    completions = failing_groq(TimeoutError("upstream gone"))
...
async def test_a_success_resets_the_failure_count(groq_stub, monkeypatch):
    monkeypatch.setenv("CLASSIFIER_CIRCUIT_FAILURES", "3")
    get_settings.cache_clear()
    ...
    monkeypatch.setattr(groq, "AsyncGroq", FailClient)
...
async def test_a_parse_failure_does_not_trip_the_breaker(groq_stub):
