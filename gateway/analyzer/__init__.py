...
from gateway.analyzer import llm, rules, trust
...
def combine(
    rule_confidence: float, llm_confidence: float, source: SourceType
) -> float:
    """Take the stronger of the two detectors, then amplify by how untrusted
    the provenance is. Deliberately not an average: averaging lets a confident
    rule hit get diluted by a hedging model, which is the wrong failure
    direction for a control that gates money."""
    base = max(rule_confidence, llm_confidence)
    if rule_confidence > 0 and llm_confidence > 0:
        # Independent corroboration is worth something.
...
async def analyze(txn: CanonicalTransaction) -> AnalysisResult:
    ...
    return AnalysisResult(
        injection_confidence=combine(rule_result.confidence, llm_result.confidence, source),
